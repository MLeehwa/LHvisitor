...
class MainVisitorSystem {
    ...
    setupAdminButton() {
        ...
        if (adminBtn) {
            console.log('관리자 버튼 찾음, 이벤트 리스너 설정');
            ...
            console.error('adminBtn 요소를 찾을 수 없습니다');
            // 1초 후 다시 시도
            setTimeout(() => {
                this.setupAdminButton();
            }, 1000);
    ...
    // 관리자 로그인 표시
    showAdminLogin() {
        console.log('showAdminLogin 메서드 호출됨');
        const password = prompt('관리자 비밀번호를 입력하세요:');
        console.log('입력된 비밀번호:', password);
        
        if (password === 'admin123') {
            console.log('비밀번호 일치, admin.html로 이동');
            // 관리자 페이지로 이동
...
window.addEventListener('load', () => {
    if (!window.mainVisitorSystem) {
        console.log('DOMContentLoaded에서 초기화 실패, window.onload에서 재시도');
