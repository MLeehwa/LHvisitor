...
class AdminSystem {
    ...
    updateLogList() {
        ...
        filteredLogs.forEach((log, index) => {
            console.log(`로그 ${index + 1} 데이터:`, log);
            ...
            if (log.name && log.name !== 'null' && log.name !== null) {
                visitorName = log.name;
            } else if (log.fullName && log.fullName !== 'null' && log.fullName !== null) {
                visitorName = log.fullName;
            } else if (log.visitorName && log.visitorName !== 'null' && log.visitorName !== null) {
                visitorName = log.visitorName;
            } else if (log.lastName || log.firstName) {
                const lastName = log.lastName || '';
                const firstName = log.firstName || '';
                visitorName = `${lastName} ${firstName}`.trim();
            }
            ...
            item.innerHTML = `
                <div class="flex items-center space-x-4 flex-1">
                    ...
                    <div class="text-sm text-gray-600 w-40">
                        <i class="fas fa-clock mr-1"></i>
                        ${log.action === 'checkin' ? `체크인: ${checkinTime}` : `체크아웃: ${checkoutTime}`}
                    </div>
                </div>
            `;
